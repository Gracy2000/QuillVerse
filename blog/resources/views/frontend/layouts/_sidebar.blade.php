<div id="sidebar" class="col-md-3 mt50 animated" data-animation="fadeInRight" data-animation-delay="250">
    ...
    <div class="pr25 pl25 clearfix">
        <form action="{{ Route::currentRouteName() == 'frontend.blogs.show' ? route('home') : '#' }}">
            <div class="blog-sidebar-form-search">
                ...
                <button type="submit" value="{{ request()->search }}" class="pull-right"><i class="fa fa-search"></i></button>
            </div>
        </form>
            ...
            @foreach ($categories as $category)
                <li class="active">
                    <a href="{{ route('frontend.category', $category->id) }}">
                        {{ $category->name }}
                        <span class="badge badge-pasific pull-right">{{ $category->blogs_count }}</span>
                    </a>
                </li>
            @endforeach
    ...
    <div class="pr25 pl25 clearfix">
        ...
        <ul class="tag">
            @foreach ($tags as $tag)
                <li>
                    <a href="{{ route('frontend.tag', $tag->id) }}">
                        {{ $tag->name }}
                    </a>
                </li>
            @endforeach
